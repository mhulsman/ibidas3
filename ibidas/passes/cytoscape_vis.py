...
networkid = util.seqgen().__next__
class DebugVisualizer(VisitorFactory(prefixes=("node",), flags=NF_ELSE),
    ...
    def run(cls, query, run_manager, portnumber=config.get('debug.cytoscape_port_number',9000)):
        ...
        self.server = xmlrpc.client.ServerProxy("http://localhost:" + str(portnumber)).Cytoscape
        ...
        self.server.createNodes(self.network, list(self.names.values()))
        for source,edges in self.graph.edge_source.items():
            for edge in edges:
        ...
        self.server.addEdgeAttributes("type","STRING",dict(list(zip(self.edgeids,self.edge_type))))
        self.server.addEdgeAttributes("attr","STRING",dict(list(zip(self.edgeids,self.edge_attr))))
        for attribute,attribute_dict in self.graph.node_attributes.items():
            attribute_name_dict = {}
            if(isinstance(list(attribute_dict.values())[0], float)):
                xtype = "FLOATING"
                cls = float
            elif(isinstance(list(attribute_dict.values())[0], int)):
                xtype = "INTEGER"
                cls = int
            else:
                xtype = "STRING"
                cls = str
            for node, node_name in self.names.items():
                try:
            ...
            if(attribute == 'links'):
                ...
                cm = discrete_color_map(list(attribute_name_dict.values()), matplotlib.cm.gist_rainbow)
