...
class Nest(repops.MultiOpRep):
    ...
    def _sprocess(self, sources, dim=None):
        ...
        for slice in rsource._slices:
            odims = slice.dims
            for dimpos in range(len(joinpath)):
...
class Match(repops.MultiOpRep):
    ...
    def _apply(cls,lsource, rsource, lslice,rslice, jointype="inner", merge_same=False, mode="dim"):
        ...
        if isinstance(merge_same, dict):
            ...
            for lname, rname in merge_same.items():
...
class Blast(repops.MultiOpRep):
    ...
    def find_seqslice(self, source, seqslice, blast_type=None):
        if seqslice is None:
            seqslice = [ i for i in range(len(source._slices)) if isinstance(source._slices[i].type, rtypes.TypeSequence) ];
        else:
            seqslice = set(source.Get(seqslice)._slices)
            seqslice = [ i for i in range(len(source._slices)) if source._slices[i] in seqslice ];
...
class Take(repops.MultiOpRep):
    def __init__(self, source, take_source, allow_missing=False, keep_missing=False, keep_name=False):
        if (isinstance(source,dict)):
             source = python.Rep(list(source.items()),name="data").IndexDict()
...
class Stack(repops.MultiOpRep):
    ...
    def _sprocess(self, sources, slices=COMMON_POS, mode='dim', dims=LCDIM, stack_missing=False, promote=False):
        ...
        if stack_missing:
            ...
            dtype_dict = {}
            for name, slicecol in slicecols.items():
                dtypes = []
