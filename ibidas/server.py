...
def Serve(portnumber=9889):
    ...
    s = xmlrpc.server.SimpleXMLRPCServer(("localhost",portnumber),allow_none=True,bind_and_activate=False, logRequests=False)
    ...
    _thread.start_new_thread(runServer,(s,))
    return s
...
class LocalRequestHandler(object):
    ...
    def execute(self, statement, sessionid=-1):
        ...
        exec(statement, getGlobals(), session)
