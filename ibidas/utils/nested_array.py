...
class NestedArray(object):
    ...
    def unpack(self, dimpath, subtype):
        ...
        while(depth>0):        
            ...
            if(rem_dims):
                ...
                nself.idxs.extend(list(range(idxdepth, idxdepth + rem_depth)))
            ...
            if(variable):
                ...
                if len(cdata):
                    pshape = int(ndata.shape[0] / len(cdata))
                else:
                    pshape = 0
    ...
    def _swapDim(self, matchpoint):#{{{
        ...
        if(isinstance(lidx,int)):
            ...
            if(isinstance(ridx,int)):  #fixed-fixed
                pidx = list(range(len(nextobj.shape)))
                tmp = pidx[lidx]
                pidx[lidx] = pidx[ridx]
                pidx[ridx] = tmp
                ...
                nnextobj = numpy.reshape(nnextobj, (int(nextobj.shape[0] / fixshape),fixshape) + nextobj.shape[1:])
                ...
                range_fixshape = list(range(fixshape))
            ...
            if(isinstance(ridx,int)): #var-fixed --> fixed-var
                ...
                pidx = list(range(len(nextobj.shape)+1))
                pidx[0] = 1
                pidx[1] = 0
                for i in range(0,len(source_lidx)):
                    ...
                    d = numpy.reshape(nextobj[rsrc],(int(leftlen),int(rightlen)) +  nextobj.shape[1:])
    ...
    def broadcast(self, repeat_dict):
        ...
        for pos,repeat  in repeat_dict.items():
            repeats[pos + 1] = repeat
        tilerepeats = []
        prev_repeat = False
        for pos,idx,repeat in zip(list(range(len(repeats))),nself.idxs,repeats):
    ...
    def _apply_tile_broadcast(self, tilerepeats, pos, prevrepeat):
        ...
        if(shapeok):
            lastrepeat = 0
            for tpos in range(len(tilerepeats)):
    ...
    def _normalize(self, curpos):
        ...
        while curpos < len(self.idxs):
            ...
            for rowpos in range(len(curobj)):
