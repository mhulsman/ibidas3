...
class Context(object):
    ...
    def __truediv__(self, other):
        return Context(self,'BINOP','__truediv__',other)
    
    def __rtruediv__(self, other):
        return Context(self,'BINOP','__rtruediv__',other)
...
def _apply(ct, obj, **extra_params):
    ...
    for action in actionlist[::-1]:
        if(action[0] == 'DOT'):
            ...
            if(extra_params):
                if(result.__name__ in extra_params):
                    kwds.update(extra_params[result.__name__])
            ...
            for p in action[2]:
                if(isinstance(p,Context)):
                    ...
                    param.append(p)
            kwds = action[3].copy()
            for k,v in action[3].items():
                if(isinstance(v,Context)):
                    kwds[k] = _apply(v, obj, **extra_params)
            if(extra_params):
                if(action[1].__name__ in extra_params):
                    kwds.update(extra_params[action[1].__name__])
