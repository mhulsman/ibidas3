...
def format_runtime(rtime):
    if rtime > 0.0 and rtime < 1000.0:
        ...
        order = 3
    return "%.*g %s" % (3, rtime * scaling[order],units[order])
...
def check_realseq(data):
    return isinstance(data,(collections.abc.Sequence, numpy.ndarray)) and not isinstance(data, str)
...
def unique_count(a):
    ...
    return dict(list(zip(unique,count)))
