...
allowed_clss = (representor.Representor, context.Context, infix.Infix, int, int, float, str)
...
class CommandLineASTRewriterPass2(GenericASTRewriter):#{{{za
    ...
    def n_call(self, node, context=None):
        ...
        if(inspect.ismethod(method)):
           if not isinstance(method.__self__, allowed_clss):
               return node
           if isinstance(method.__self__, representor.Representor) and method.__func__.__name__ in ['__str__','__nonzero__','Copy','__reduce__']:
               return node
