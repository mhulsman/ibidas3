...
class Representor(Node):
    ...
    def __str__(self, print_data=True, table_length=None):
        ...
        for i in range(len(indices)):
            ...
            nwidths = numpy.asarray(tableprint.optimize_width(console_width, numpy.asarray(widths, dtype=float), 3),dtype=int)
    ...
    def __getattr__(self, name):
        ...
        if(not axis_letter.upper() == axis_letter or axis_letter == "_"):
            try:
                ...
                raise RuntimeError(str(error)).with_traceback(traceback)
            ...
            try:
                ...
                #(thereby raising a new exception)
                exc_class, exc, traceback = sys.exc_info()
                raise RuntimeError(str(error) + " (and there is also no method '" + axis_letter + name + "')").with_traceback(traceback)
        else:
