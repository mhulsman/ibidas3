...
class UnaryOp(Op):#{{{
    ...
    def __init__(self, name, rtype = rtypes.unknown, dims=dimpaths.DimPath(), bookmarks=set()):
        ...
        assert all([isinstance(bm, str) for bm in bookmarks]),"Bookmarks should be a string"
...
def apply_broadcast_plan(slice,plan, bcdims, references, partial):#{{{
    nplan = []
    active_bcdims = []
    for dimpos, bcdim, planelem in zip(list(range(len(bcdims))),bcdims, plan):
        if(planelem == BCNEW):
...
class EachOp(MultiUnaryOp):
    ...
    def __init__(self, source_slices, eachfunc, rtype=rtypes.unknown, named_params=False, keep_name=False, extra_params={}):
        ...
        if not keep_name and not isinstance(eachfunc, context.Context) and hasattr(eachfunc,'func_name') and not eachfunc.__name__=='<lambda>':
            name = util.valid_name(eachfunc.__name__)
        else:
...
class PackTupleOp(MultiUnaryOp):#{{{
    ...
    def __init__(self, slices, field="data"):
        ...
        MultiUnaryOp.__init__(self, slices, name=field, rtype=ntype, dims=next(iter(cdim)),bookmarks=nbookmarks)#}}}
...
class HArrayOp(MultiUnaryOp):#{{{
    ...
    def __init__(self, slices, dimname=None, slicename=None):
        ...
        MultiUnaryOp.__init__(self, slices, name=slicename, rtype=ntype, dims=next(iter(cdim)),bookmarks=nbookmarks)#}}}
...
class BlastIndexOp(MultiMultiOp):#{{{
    ...
    def __init__(self, leftslice, rightslice, blast_type, folder, algorithm, **kwargs):
        ...
        r = tuple([ SelectOp(self, i, names[i], slice_types[i] ,leftslice.dims, set([])) for i in range(len(slice_types)) ]);
