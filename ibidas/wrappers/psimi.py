...
class PSIMIOp(ops.ExtendOp):
    ...
    def py_exec(self):
        ...
        file.close()
        exps = [Experiment(**exp) for exp in list(psimi_handler.experiments.values())]
        interactors = [Interactor(**inter) for inter in list(psimi_handler.interactors.values())]
        interactions = []
        for interaction in list(psimi_handler.interactions.values()):
