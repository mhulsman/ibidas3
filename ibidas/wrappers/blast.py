...
def blast(data, type, folder, reciprocal = True, normalize = False, overwrite = False, blastopts='-num_threads %d' % multiprocessing.cpu_count()):
  ...
  title_1 = [ "%d" % i for i in range(len(seq_1)) ];
  title_2 = [ "%d" % i for i in range(len(seq_2)) ];
  ...
  ab = [ [ list(p[0]) + h for h in p[1] ] for p in list(ab.items()) ];
  ...
  return tuple([ util.darray(row,type) for (type,row) in zip( sp_types, list(map(lambda *row: list(row), *ab))) ] );
...
def blast_res_to_dict(blast_res, max=False):
  ...
  for (i, row) in enumerate(rdr):
    row = [ sp_types[i](row[i]) for i in range(len(row)) ];
    k = (row[0], row[1]);
    if max == True:
      if k in bm:
