...
class PyExec(VisitorFactory(prefixes=("visit","unpackCast"), flags=NF_ELSE), 
    ...
    def run(cls, query, run_manager,debug_mode=False):
        ...
        for command_id, command in enumerate(commands):
            ...
            param_kwds = dict()
            for name,param_ids in params.items():
    ...
    def visitUnpackTupleOp(self,node,slice):
        if(isinstance(slice.type,rtypes.TypeRecordDict)):
            ...
            if node.tuple_idx == 0:
                func = lambda x: list(x.keys())
            else:
                func = lambda x: list(x.values())
        else:
            if(node.type.has_missing):
                def func(x):
    ...
    def visitPackDictOp(self,node, slices):
        ...
        if node.with_missing or not any([slice.type.has_missing for slice in slices]):
            def speeddictify(x):
                return util.darray([dict(list(zip(names,row))) for row in zip(*x)])
        else:
            def speeddictify(x):
                non_hasmissing = []
                ...
                if(non_hasmissing):
                    ...
                    d = [dict(list(zip(snames,row))) for row in zip(*nx)]
                else:
                    d = [{} for i in range(len(x[0]))]
                for pos, slice in enumerate(slices):
                    if(pos in non_hasmissing):
                        continue
                    name = names[pos]
                    val = x[pos]
                    for rowpos in range(len(val)):
                        elem = val[rowpos]
    ...
    def castto_pickle(self,castname,node,slice):
        dtype = node.type.toNumpy()
        return slice.data.map(lambda x:pickle.dumps(x,2),res_type=node.type)
    
    def castfrom_pickle(self,castname,node,slice):
        dtype = node.type.toNumpy()
        return slice.data.map(lambda x:pickle.loads(str(x)),res_type=node.type)
...
def speedtuplify(seqs):
    nseq = util.darray(list(zip(*seqs)))
    return nseq
    ...
    if extra_params:
        ...
        if named_params is False:
            ...
            for args in zip(*seqs):
                args = dict(list(zip(named_params, args)))
...
def speedarrayify(seqs,dtype):
    if all([isinstance(s, numpy.ndarray) for s in seqs]) and len(set([s.shape for s in seqs])) > 1:
        ...
        nseq = numpy.array(seqs,dtype).transpose([1,0] + list(range(2, len(seqs[0].shape)+1)))
    return nseq
...
def groupindex(data):
    ...
    if(len(data) > 1):
        ...
        for key, posses in data_dict.items():
        ...
        for pos, elems in enumerate(*data):
            if(elems is Missing):
                ...
                data_dict[elems].append(pos)
        indexdata = [util.darray(elem, int) for elem in list(data_dict.values())]
...
def joinindex(data, jointype):
    ...
    for elem,lpos in lelempos.items():
