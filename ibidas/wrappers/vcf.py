...
class VCFRepresentor(wrapper.SourceRepresentor):
    def __init__(self, filename):
        ...
        for hasmissing, shape, type in zip(hasmissing, shapes,types):
            ...
            for pos, s in zip(list(range(len(shape)))[::-1], shape[::-1]):
                if isinstance(s, int):
                    if s <= 1:
                        continue
                    dim = dimensions.Dim(shape=s, name=next(dimnames))
                else:
                    s = s.lower()
...
class VCFParser(object):
    ...
    def processLine(self, line):
        ...
        if self.has_genotypes:
            ...
            format = list(zip(*[fill_in(elem.split(':'),nformat) for elem in format]))
