...
class Connection(object):
    ...
    def _getAttributeNames(self):
        return list(self._tabledict.keys())
    ...
    def __repr__(self):
        ...
        tablenames = list(self._tabledict.keys())
        tablenames.sort()
        if tablenames:
        ...
        if(not name in self._tabledict):
            ...
            for pos, slice in enumerate(rep._slices):
                ...
                if(slice.dims):
                    ...
                    dimname = Missing
                    pickled = True
                    val = rep.Get(slice.name).Cast("pickle").Each(memoryview,dtype="pickle").ToPython()
            ...
            if NeedInfo: #(len(columns) > 1 or any([row['pickle'] or row['dimname'] is None for row in rowinfo])):
                ...
                self._sa_tables[name] = list(tables.values())[0]
    ...
    def _append(self, name, rep):#{{{
        ...
        if(isinstance(tbl,CombineRepresentor)):
            ...
            rep = rep.To(*pickle, Do=_.Cast("pickle").Each(memoryview,dtype="pickle"))
            rep = rep.Copy()
...
class SQLPlanner(VisitorFactory(prefixes=("eat","expressionEat"), 
    ...
    def finish(self, node):#{{{
        ...
        rslices = []
        for oslice, i in zip(results, list(range(len(results)))):
...
class Table(object):#{{{
    def __init__(self, source_descriptor, column_names):
        ...
        self.colids = list(range(len(list(column_names))))
        self.column_names = column_names
        self._alias = self.getName() + str(next(alias_name_id)) 
...
class TableQuery(Table):
    def __init__(self, source_descriptor, columns):
        self.alias = 'query' + str(next(alias_name_id)) 
...
class AliasTable(Table):#{{{
    def __init__(self, origtable):
        ...
        self._alias = origtable.getName() + str(next(alias_name_id))
...
class Query(Element):#{{{
    ...
    def _clause_subquery(self, clause):
        ...
        column_dict = dict(list(zip(self.columns,nself.columns)))
