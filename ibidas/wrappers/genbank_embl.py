...
class GERepresentor(wrapper.SourceRepresentor):
    ...
    def _getRecordRep(self, records, records_dim):
        keys = list(set(chain(*[list(r.keys()) for r in records])))
        keys.sort()
        types = []
        dims = {}
        for key in keys:
...
featrec = ZeroOrMore(Group((LineStart() | Word('\n')) +  Suppress('/') + Word(alphas + '_') + Optional(Suppress('=') + (Word(nums).setParseAction(lambda x: list(map(int,x))) | QuotedString(quoteChar='"',  escQuote='""', multiline=True).setParseAction(lambda x: [e.replace('\n',' ') for e in x])))))
...
#location
number = Word(nums).setParseAction(lambda x: list(map(int,x)))
