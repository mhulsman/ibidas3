...
def read_fasta(fname, sep='auto', fieldnames=()):
    ...
    if sep == 'auto':
        ...
        for sep in sep_search:
            if all([sep in fas[i][0] for i in range(min(len(fas),100))]):
                break
        else:
    ...
    if sep:
        ...
        fas = [ tuple([ fsid[i][j] if j < len(fsid[i]) else '' for j in range(maxf)]) + (seqs[i],) for i in range(len(fas)) ]
    else:
        fas = [ (fas[i], seqs[i]) for i in range(len(fas)) ];
        maxf = 1
...
def save_fasta_rep(data, fout, **kwargs):
  ...
  names = data.Names;
  if seqslice is None:
    seqslice = [ i for i in range(len(data._slices)) if isinstance(data._slices[i].type, rtypes.TypeSequence) ];
  else:
    seqslice = set(data.Get(seqslice)._slices)
    seqslice = [ i for i in range(len(data._slices)) if data._slices[i] in seqslice ];
  #fi
...
def read_fastq(fname, **kwargs):
  print(fname)
  fd = util.open_file(fname, mode='r');
