...
class TypeArray(TypeAny):#{{{
    ...
    def toDefval(self):
        ...
        if(isinstance(subdv, collections.abc.Sequence)):
            flatres = res.ravel()
...
class TypeStringScanner(GenericScanner):#{{{
    ...
    def t_name(self,s):
        r'[a-zA-Z_][a-zA-Z_\d]*'
        t = Token(kind='name',attr=s)
        self.rv.append(t)

    def t_integer(self,s):
        r'\d+'
        t = Token(kind="integer",attr=int(s))
        self.rv.append(t)
...
class TypeStringParser(GenericParser):#{{{
    ...
    def p_param_1(self,args):
        '   param ::= @ name '
        return AST(kind="param",kids=args[1:])
    ...
    def p_tmatcher(self, args):
        ...
        """
        if len(args) > 1:
            return AST(kind="tmatcher",kids=args)
        else:
            return args[0]
    ...
    def p_tmatcher_list(self, args):
        ...
        """
        return AST(kind="type_orlist",kids=args[:1] + args[2:])
    ...
    def p_type_match_0(self, args):
        ...
        """
        return AST(kind="type_andlist", kids=args[:1] + args[2:])
   
    def p_type_andlist_1(self, args):
        """
            typenest ::= # type_andlist #
        """
        return AST(kind="typenest", kids=args[1:2])
    ...
    def p_dmatcher(self, args):
        ...
        """
        if len(args) > 1:
            return AST(kind="dmatcher",kids=args)
        else:
            return args[0]
    ...
    def p_dmatcher_list(self, args):
        ...
        """
        return AST(kind="dim_orlist",kids=args[:1] + args[2:])
    ...
    def p_dim_match_0(self, args):
        ...
        """
        return AST(kind="dim_andlist", kids=args[:1] + args[2:])

    def p_dim_andlist(self, args):
        """
            nameddim ::= # dim_andlist #
        """
        return AST(kind="nameddim", kids=args[1:2])
    ...
    def p_param_call(self, args):
        """
            ...
            param ::= param ( dimlist )
        """
        return AST(kind="param_call", kids=[args[0], args[2]])
    ...
    def p_type_1(self,args):
        """
            ...
            type ::= QUESTION
        """
        return AST(kind="createtype",kids=args[:1])
    ...
    def p_type_2(self,args):
        """
            ...
            type ::= type !
        """
        return AST(kind="hasmissing", kids=args)
    
    def p_type_2b(self,args):
        """
            type ::= type '
        """
        return AST(kind="strict", kids=args)
   
    def p_type_3(self,args):
        ' type ::= type [ dimlist ] '
        return AST(kind="dims",kids=(args[0], args[2]))
    
    def p_type_3b(self,args):
        ' type ::= type [ ] '
        return AST(kind="dims",kids=(args[0],))

    def p_type_4(self,args):
        ' type ::= type ( typelist ) '
        return AST(kind="subtypes",kids=(args[0], args[2]))
    
    def p_type_6(self,args):
        ' type ::= [ dimlist ] '
        return AST(kind="dims",kids=(
            AST(kind="createtype", kids=(Token(kind="name",attr="array"),)), args[1]))
    
    def p_type_7(self,args):
        ' type ::= { dimlist } '
        return AST(kind="dims",kids=(
            AST(kind="createtype", kids=(Token(kind="name",attr="set"),)), args[1]))
    ...
    def p_type_8(self,args):
        ...
        if len(args) == 1:
            ...
            return AST(kind="typeelem",kids=(args[2], args[0]))
    
    def p_type_9(self,args):
        ' type ::= ( typelist ) '
        return AST(kind="subtypes",kids=(
            AST(kind="createtype", kids=(Token(kind="name",attr="tuple"),)), args[1]))
    
    def p_type_9b(self,args):
        ' type ::= { typelist } '
        return AST(kind="subtypes",kids=(
            AST(kind="createtype", kids=(Token(kind="name",attr="record_dict"),)), args[1]))
    ...
    def p_typenest_2(self,args):
        ...
        """
        return AST(kind="typenest",kids=args)
    ...
    def p_varlist_1(self,args):
        ''' 
            varlist ::= var
        '''
        return AST(kind="varlist",kids=args[:1])
    
    def p_varlist_2(self,args):
        ''' 
            varlist ::= varlist var
        '''
        return AST(kind="varlist",kids=args)
    ...
    def p_dim_1(self,args):
        ''' 
            ...
            dim ::= param
        '''
        return AST(kind="createdim",kids=args)


    def p_nameddim_1(self,args):
        ''' nameddim ::= name : dim '''
        return AST(kind="namedim",kids=(args[2], args[0]))
    ...
    def p_nameddim_3(self,args):
        ''' nameddim ::= name '''
        return AST(kind="namedim",kids=(
            AST(kind="createdim", kids=(Token(kind="inherit"),)), args[0]))
   
    def p_nameddim_4(self,args):
        ''' 
            nameddim ::= nameddim  $
            nameddim ::= nameddim  QUESTION
            nameddim ::= nameddim  !
        '''
        return AST(kind="hasmissing",kids=args)
    ...
    def p_dimlist_1(self,args):
        ...
        '''
        return AST(kind="dimlist",kids=args[:1] + args[2:])

    def p_typelist_1(self,args):
        ''' typelist ::= typeelem 
            typelist ::= typelist , typeelem '''
        return AST(kind="typelist", kids=(args[:1] + args[2:]))#}}}
...
class TypeStringASTRewriterPass1(GenericASTRewriter):#{{{
    ...
    def collapse_list(listname):
        def func(self, node):
            if(len(node) > 1):
                if node[0].kind == listname:
                    node[:] = tuple(node[0]) + tuple(node[1:])
                return node 
            else:
                return node[0]
        return func
    ...
    def n_dimlist(self, node):
        if(node[0].kind == "dimlist"):
            node[:] =tuple(node[0]) + (self.annotateDim(node[1]),)
        else:
            node[:] = (self.annotateDim(node[0]),)
        return node
  
    def n_varlist(self, node):
        if(node[0].kind == "varlist"):
            node[:] = tuple(node[0]) + (self.processVar(node[1]),)
        else:
            node[:] = (self.processVar(node[0]),)
        return node
    ...
    def processVar(self,node):
        if(node.kind == '.'):
            ...
            raise RuntimeError("Unexpected character as dim var: "+ node.kind)

    def n_typenest(self,node):
        if(len(node) > 1):
            if node[2].kind == "typenest":
                node.packed = node[0].packed + [node[1].kind == "<"] + node[2].packed
                node[:] = tuple(node[0]) + tuple(node[2])
            else:
                node.packed = node[0].packed + [node[1].kind == "<"]
                node[:] = tuple(node[0]) + (node[2],)
        else:
            node.packed = []
            node[:] = (node[0],)
        return node
    ...
    def n_createtype(self, node):
        node[:] = [node[0], None, None, None, None, False] #hasmissing, name, dims, subtypes, strict
        return node
  
    def n_createdim(self, node):
        node[:] = [node[0], None, None, False] #hasmissing, name, strict
        return node

    def n_hasmissing(self, node):
        if(node[0].kind == "createtype"):
            node[0][1] = node[1].kind in ("QUESTION","$")
        elif(node[0].kind == "createdim"):
            node[0][1] = node[1].kind in ("QUESTION","$")
        else:
    ...
    def n_dims(self, node):
        assert node[0].kind == "createtype", "Invalid AST!"
        if(len(node) > 1):
            assert node[1].kind == "dimlist", "Invalid AST!"
            node[0][3] = node[1]
        return node[0]

    def n_subtypes(self, node):
        assert node[0].kind == "createtype", "Invalid AST!"
        assert node[1].kind == "typelist", "Invalid AST!"
        node[0][4] = node[1]
        return node[0]
    
    def n_namedim(self, node):
        if(node[0].kind == "createdim"):
            node[0][2] = node[1].attr
        else:
            return node
        return node[0]
    ...
    def annotateDim(self,node):
        if not node.kind == "createdim":
            return node
       
        depshape, has_missing, name, strict = node
        if(name is None):
            return node

        if(depshape.kind == "varlist"):
            dependent = list(depshape)
            while(dependent and dependent[-1] is False):
                dependent = dependent[:-1]
            dependent = tuple(dependent)
            shape = UNDEFINED
        elif(depshape.kind == "integer"):
            dependent = tuple()
            shape = depshape.attr
        elif(depshape.kind == "~"):
            dependent = "~"
            shape = UNDEFINED
        elif(depshape.kind == "inherit"):
            dependent = None
            shape = None
        else:
            raise RuntimeError("Invalid AST!")
...
class TypeStringASTRewriterPass2(GenericASTRewriter):#{{{
    ...
    def n_typenest(self,node):
        ...
        while(len(kids) > 1):
            right = kids.pop()
            assert right.kind == "createtype", "Invalid AST!"
            assert kids[-1].kind == "createtype", "Invalid AST!"
            kids[-1][4]= AST(kind="typelist",kids=(right,))
...
class TypeStringASTInterpreter(object):#{{{
    ...
    def n_createtype(self, node, dimpos=0):
        assert node.kind == "createtype", "Cannot create type from this specification!"
      
        typetoken, has_missing, name, dimlist, subtypelist, strict = node

        if has_missing is None:
            has_missing = False

        if(typetoken.kind == 'QUESTION'):
            typename = "?"
        else:
        ...
        if(not dimlist is None):
            ...
            dims = dimpaths.DimPath(*[self.visit(dimnode, dimpos + pos) for pos, dimnode in enumerate(dimlist)])
            dimpos += len(dims)
            kwargs['dims'] = dims
        ...
        if not subtypelist is None:       
            subtypes = tuple([self.visit(subtypenode, dimpos) for subtypenode in subtypelist])
            kwargs['subtypes'] = subtypes

        if(issubclass(typecls,TypeTuple)):
            fieldnames = []
            for pos, subtypenode in enumerate(subtypelist):
                if subtypenode.kind == "createtype":
                    name = subtypenode[2]
                elif subtypenode.kind == "typeelem":
                    name = subtypenode[1].attr
                else:
                    name = None
                if name is None:
    ...
    def n_typeelem(self, node, dimpos):
        return self.visit(node[0],dimpos)

    def n_param(self, node,dimpos):
        if not node[0].attr in self.env:
            raise RuntimeError("Cannot find variable '" + str(node[0].attr) + "' in given variables")
        return self.env[node[0].attr]

    def n_createdim(self, node, dimpos):
        assert node.kind == "createdim", "Cannot create dim from this specification!"
        dimnode, has_missing, name, strict = node
        if has_missing is None:
            has_missing = False
        
        if(dimnode.kind == "varlist"):
            dependent = tuple(dimnode)
            shape = UNDEFINED
        elif(dimnode.kind == "integer"):
            dependent = tuple()
            shape = dimnode.attr
        elif(dimnode.kind == "~"):
            dependent = (True,) * dimpos
            shape = UNDEFINED
        elif(dimnode.kind == "inherit"):
            dependent = None
            shape = None
        else:
            raise RuntimeError("Invalid AST!")
...
def _createType(name, dimpos=0, refdims=[], env={}):
    ...
    #print tree
    rewriter1 = TypeStringASTRewriterPass1(tree)
    tree, dim_annotation = rewriter1.process(tree)
    #print tree, dim_annotation
    rewriter2 = TypeStringASTRewriterPass2(tree)
...
class TypeStringMatchASTInterpreter(object):
    ...
    def n_typenest(self, node, matchtype, dims=None):
        assert node.kind == "typenest", "Invalid AST!"
        packlist = node.packed + [False]
        for packed, kid in zip(packlist[::-1], node[::-1]):
            if packed:
        ...
        for pos, (packed, matcher) in enumerate(zip(packlist, node)):
            if packed:
                continue
    ...
    def n_type_orlist(self, node, matchtype):
        results = [self.visit(matcher, matchtype) for matcher in node]
    ...
    def n_tmatcher(self, node, matchtype):
        if node[0].kind == "^":
            res,ms = self.visit(node[1], matchtype)
            res = not res
            if res is False:
            ...
            return self.visit(node[0], matchtype)
    ...
    def n_createtype(self, node, matchtype):
        ...
        if not subtypelist is None:
            if not hasattr(matchtype, "subtypes"):
                ...
                return (False,())
            if subtypelist.kind == "typelist":
                if not len(subtypelist) == len(matchtype.subtypes):
                    self.setError("Incorect number of subtypes, expected: " + str(len(subtypelist)) + " but found: " + str(len(matchtype.subtypes)))
                    return (False,())
                for subtype, kid in zip(matchtype.subtypes, subtypelist):
                    res,ms = self.visit(kid, subtype)
                    if res is False:
                        return (False,())
            elif subtypelist.kind == "createtype":
    ...
    def n_dimlist(self, node, dims):
        return self.match(node, dims)
    
    def n_nameddim(self, node, dims):
        return self.visit(node[0], (dims,))
    
    def n_dim_orlist(self, node, dims):
        xdims = (dims,)
        results = [self.visit(matcher, xdims) for matcher in node]
        res = any([x for x, m in results])
        ms = sum([m for x, m in results if x],())
        if res is True:
            self.unsetError()
    ...
    def match(self, dim_matchers, dims):
        ...
        if dim_matchers[0].kind == "QUESTION":
            if len(dims) == 0:
                self.setError("Expected dimension, but found none")
                return (False,())
            return self.match(dim_matchers[1:], dims[1:])
        elif dim_matchers[0].kind == '??':
            mres = [self.match(dim_matchers[1:],dims[pos:])  for pos in range(len(dims) + 1)]
            res = any([x for x,y in mres])
            if res is True:
                self.unsetError()
            return (res,())
