    ...
    def updateDepDim(self, pos, ndim):
        ...
        if(not key in redim_cache):
            ...
            if nself.dependent[pos]:
                ndependent = tuple([a or b for a,b in itertools.zip_longest(ndim[0].dependent, nself.dependent[(pos+1):],fillvalue=False)])
    ...
    def merge(self, other):
        ...
        if not key in redim_cache:
            if(self.shape == UNDEFINED or other.shape == UNDEFINED) or self.shape != other.shape:
                rshape = UNDEFINED
                ndep = tuple([ldep or rdep for ldep, rdep in itertools.zip_longest(self.dependent, other.dependent,fillvalue=False)])
                if not ndep:
