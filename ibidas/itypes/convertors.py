...
class DictConvertor(BaseConvertor):
    def convert(self,seq,elem_type):
        ...
        return util.darray(list(zip(*columns)))
        ...
        if(ptype.has_missing):
            ...
            for elem in seq:
                if elem is Missing:
                    ...
                    r = dict([(key,self.fconvert(value)) for key,value in elem.items()])
                seqres.append(r)
        else:
            seqres = [dict([(key, self.fconvert(value)) for key,value in elem.items()]) for elem in seq]
