...
class DimPath(tuple):
    ...
    def removeDim(self, pos, elem_specifier=None, subtype=None, has_missing=False):#{{{
        ndims = []
        for p in range(max(pos + 1,0), len(self)):
    ...
    def insertDim(self, pos, ndim, subtype=None):#{{{
        ...
        for p in range(max(pos,0), len(self)):
        ...
        if(prevdims):
            ...
            for pos, dim in enumerate(self):
                ...
                if(len(dep) > pos - permute_start):
                    deppos = util.select(list(range(-len(prevdims),pos))[::-1],dep)
            ...
            for pos, permute_idx in enumerate(permute_idxs):
                ...
                dep = dim.dependent
                if(dep):
                    deppos = util.select(list(range(0,permute_idx))[::-1],dep)
                    ndep = [False] * pos
                    for dp in deppos:
...
def uniqueDimPath(dimpaths,only_unique=True):#{{{
    ...
    if(len(fdims) == 1):
        ...
        path = []
        for elems in itertools.zip_longest(*fdims):
...
def orderAllDimPath(dimpaths):#{{{
    ...
    for dp in dimpaths:
        ...
        for pos, d in enumerate(dp[1:]):
            ...
            except toposort.CycleError as e:
                pass
...
def planBroadcastMatchDim(paths):#{{{
    ...
    rev_translate = dict(sum([[(subid,dim) for subid in id] for dim,id in translate.items()],[]))
    bcdims = list(graph)
    ...
    for planpos in range(len(plans[0])):
...
def createDimParentDict(sourcepaths):#{{{
    ...
    for dimpath in sourcepaths:
        ...
        for pos in range(1,len(dimpath)):
...
def deconvertDoubleDim(paths, translate):
    translate = dict([(v,k) for k,v in translate.items()])
    npaths = []
    for path in paths:
        npath = []
        for dim in path:
