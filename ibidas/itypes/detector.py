...
.. rubric:: Overview
    ...
    and then during data processing goes up in the hierarchy if the more specific
    ...
    def processSeq(self, seq):
        if isinstance(seq,numpy.ndarray) and len(seq.shape) == 1:
            ...
            if dtype == numpy.object:
                ...
                seq = seq.ravel()[:1] # single element for scanning sufficient
        ...
        if not objclasses.issubset(missing_cls):
            ...
            if len(seq) > 0:
                for pos in range(len(self._scanners) - 1, -1, -1):
...
class DimEqualizer(object):
    ...
    def processDims(self):
        self.dimid = util.seqgen().__next__
        for dr in self.dimreps:
            if dr.dirty:
                self._attachDim(dr)
...
class TupleScanner(TypeScanner):
    ...
    def getType(self):
        if(not len(self.fieldnames) == self.max_len):
            self.fieldnames = ['f' + str(i) for i in range(self.max_len)]
        subtypes = tuple([self.getSubDetector(i).getType() for i in range(self.max_len)])
...
class ContainerScanner(TypeScanner):
    ...
    def getType(self):
        ...
        dims = dimpaths.DimPath(*[self.getDimRep(i).dim for i in range(self.min_dim)])
    ...
    def scan(self, seq):
        ...
        if not self.detector.objectclss.issubset(self.good_cls):
            ...
            l = seq.map(lambda x: isinstance(x, collections.abc.Sequence) and hasattr(x,'__len__'), otype=bool, out_empty=True, has_missing=has_missing)
        ...
        if self.min_dim == 1 and not has_missing and len(self.detector.objectclss) == 1 and numpy.ndarray in self.detector.objectclss:
            dtypes = set([elem for elem in seq.map(lambda x: x.dtype, otype=object, out_empty=None, has_missing=has_missing).ravel() if not elem is None])
...
class NumberScanner(TypeScanner):
    ...
    good_cls = set((bool, float, complex, int, int, numpy.int8, numpy.int16, numpy.int32, numpy.int64, numpy.uint8, numpy.uint16, numpy.uint32, numpy.uint64, numpy.float32, numpy.float64, numpy.complex64, numpy.complex128, numpy.bool_, MissingType))
...
class RealScanner(NumberScanner):
    ...
    good_cls = set((float, bool, numpy.float32, numpy.float64, MissingType, int, int, numpy.int8, numpy.int16, numpy.int32, numpy.int64, numpy.uint8, numpy.uint16, numpy.uint32, numpy.uint64, numpy.bool_))
registerTypeScanner(RealScanner)


class IntegerScanner(TypeScanner):
    parentcls = RealScanner
    ustepvals = [256, 65536, 4294967296, 18446744073709551616]
    uinttypes = [rtypes.TypeUInt8, rtypes.TypeUInt16, rtypes.TypeUInt32, rtypes.TypeUInt64]
    istepvals = [128, 32768, 2147483648, 9223372036854775808]
    inttypes = [rtypes.TypeInt8, rtypes.TypeInt16, rtypes.TypeInt32, rtypes.TypeInt64]
    
    good_cls = set((bool, int, int, numpy.int8, numpy.int16, numpy.int32, numpy.int64, numpy.uint8, numpy.uint16, numpy.uint32, numpy.uint64, numpy.bool_, MissingType))
    
    numpy_minmax = {numpy.dtype('bool'): (0, 1), numpy.dtype('int8'): (-128, 127), numpy.dtype('uint8'): (0, 255), numpy.dtype('int16'): (-32768, 32767), numpy.dtype('uint16'): (0, 65535), numpy.dtype('int32'): (-2147483648, 2147483647), numpy.dtype('uint32'): (0, 4294967295), numpy.dtype('int64'): (-9223372036854775808, 9223372036854775807), numpy.dtype('uint64'): (0, 18446744073709551615)}
    array_minmax = {'b': (-128, 127), 'B': (0, 255), 'h': (-32768, 32767), 'H': (0, 65535), 'i': (-2147483648, 2147483647), 'I': (0, 4294967295), 'l': (-2147483648, 2147483647), 'L': (0, 4294967295)}
