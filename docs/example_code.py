def load_arrayexpress():
    if(os.path.exists('array_express.data')):
        ...
        ae_down = numpy.array(list(map(list, aexpress.expression[_ == "DOWN"].count()())),dtype=int)
        ae_up = numpy.array(list(map(list, aexpress.expression[_ == "UP"].count()())),dtype=int)
...
def add_protprot_edges(network, gn_to_orf):
    if(os.path.exists('protprot.data')):
        ...
        dres = numpy.concatenate((numpy.array(list(map(list,res[3])), dtype="uint16"), res[4], res[5]), axis=1)        
...
def addprotein_dna_edges(network, gn_to_orf):
    if(os.path.exists('protdna.data')):
        ...
        for tpos in range(len(set_ccdata)):
